...
_The aim of this article is to complement the above-mentioned paper, and some level of paraphrasing is to be expected. In this perspective, the paper's notation and equation numbering have been strictly followed so that one may read the paper alongside this post without confusion. Equations with lowercase Roman numeral numbering have been introduced in this article for explanatory purposes, and do not appear in the paper._
...
##### Notation
Let $$\mathbb{R}$$, $$\mathbb{R}_+$$, and $$\mathbb{R}_{++}$$ be the set of real, nonnegative, and positive numbers. The set $$\{1,...,n\}$$ is denoted by $$\left[n\right]$$. $$I_n$$ denotes the $$n$$-dimensional identity matrix. $$\mathbb{S}^n$$  denotes the space of $$n\times n$$ symmetric matrices. The gradient of a function $$f\left(x,t\right):\mathbb{R}^n\times\mathbb{R}_+\rightarrow\mathbb{R}$$ with respect to $$x\in\mathbb{R}^n$$ is denoted by $$\nabla_x f\left(x,t\right)$$. The _partial_ derivatives of $$\nabla_x f\left(x,t\right)$$ with respect to $$x$$ and $$t$$ are denoted by $$\nabla_{xx} f\left(x,t\right)$$ and $$\nabla_{xt} f\left(x,t\right)$$, respectively.
The variable $$x\in\mathbb{R}^n$$ is considered, with $$t\ge 0$$ a continuous time index. A time-varying objective function is defined as $$f_0 :\mathbb{R}^n\times\mathbb{R}_+\rightarrow\mathbb{R}$$ taking values $$f_0 \left(x,t\right)$$; $$p$$ time-varying inequality constraint functions are defined as $$f_i :\mathbb{R}^n\times\mathbb{R}_+\rightarrow\mathbb{R}$$ taking values $$f_i \left(x,t\right)$$ for $$i\in\left[p\right]$$; and $$q$$ time-varying affine equality constraint functions are defined as $$f_i^\prime :\mathbb{R}^n\times\mathbb{R}_+\rightarrow\mathbb{R}$$ taking values $$f_i^\prime = a_i\left(t\right)^T x-b_i\left(t\right)$$ for $$i\in\left[q\right]$$. It is assumed that $$f_i\left(x,t\right),\, i\in\{0\}\cup\left[p\right]$$ and $$f_i^\prime\left(x,t\right),\, i\in\left[q\right]$$ are twice continuously differentiable with respect to $$x$$ and piecewise continuously differentiable with respect to $$t$$, for all $$\left(x,t\right)\in\mathbb{R}^n\times\mathbb{R}_+$$.
